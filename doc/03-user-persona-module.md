...
### 4.5 【重点标记】已知 Bug 修复记录

**2026-05-12 - 修复用户头像存储路径错误**：
- **问题**：`avatarService.ts` 构造函数使用 `process.cwd()` 作为基础路径，导致头像目录生成类似 `G:\AI\creative-cafe\data\user-avatars` 的绝对路径，而非正确的 Electron 用户数据目录 `__USER_DATA__/data/user-avatars`
- **影响**：打包后应用无法正确访问头像目录，头像文件存储位置不符合 Electron 规范
- **修复方案**：
  1. 从 `../utils/appPath` 导入 `getUserDataPath`
  2. 将构造函数中的 `process.cwd()` 替换为 `getUserDataPath()`
  3. 头像目录现在正确设置为 `path.join(getUserDataPath(), 'data', 'user-avatars')`
- **涉及文件**：`src/main/services/avatarService.ts`
- **参考文件**：`src/main/services/pathService.ts`（已正确使用 `getUserDataPath()`）
