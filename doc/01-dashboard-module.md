...
| 使用技巧 | JSON 文件 | `data/tips.json`（项目根目录下的 data 目录） |
...
- **使用技巧**: 通过 `tipsCacheRef` 进行内存缓存，组件重新挂载时优先使用缓存数据

### 6.3 Tips 内容说明

Tips 文件存储在 `data/tips.json`，通过 `fileService.readJsonFile('tips')` 读取（IPC: `file:readJson`），文件路径解析为 `{projectRoot}/data/tips.json`。

**当前 Tips 内容结构**（截至 2026-05-10 更新）：

| ID | 标题 | 类别 | 覆盖内容 |
|----|------|------|---------|
| 1 | 🚀 快速上手导航 | 导航说明 | 11 个核心模块功能概览 |
| 2 | ⚙️ 配置 AI 引擎（首要步骤） | 关键配置 | AI 引擎配置流程与验证 |
| 3 | 📚 世界书高效编辑 | 功能亮点 | 条目管理、AI 辅助功能、分页浏览 |
| 4 | 🎭 角色卡创作与测试闭环 | 工作流 | 创作→测试→优化完整闭环 |
| 5 | 💬 角色对话测试技巧 | 功能亮点 | 流式响应、续写、重试、参数调节 |
| 6 | 🧠 知识库与语义检索 | 功能亮点 | 文档上传、分块向量化、对话上下文增强 |
| 7 | 👤 用户人设管理 | 导航说明 | 多身份管理、头像格式支持 |
| 8 | ✨ 效率提升技巧 | 效率技巧 | 5 条实用效率提升技巧 |
| 9 | 🔄 数据持久化与安全 | 安全说明 | 数据持久化机制、API 密钥安全 |
| 10 | 🎯 典型工作流推荐 | 工作流 | 完整角色创作流程推荐 |

**维护规则**：
- 新增功能模块或重大改动时，及时补充/修改对应 Tips
- 未来可考虑根据系统语言切换加载对应的 tips.json（如 `tips-zh.json`、`tips-en.json`）
