...
import { Modal, Steps, Checkbox, Button, Card, Typography, message, Input, Spin, Space, Tag, Form } from 'antd';
import { ThunderboltOutlined, LoadingOutlined, CheckCircleOutlined } from '@ant-design/icons';
import { createDefaultEntry, parseAIJsonResponse } from '../../utils/worldBookUtils';
...
interface WorldBookGenerateModalProps {
  ...
  onGenerateFromCharacters: (charactersInfo: string) => Promise<{ name: string; description: string; entries: any[] }>;
}
...
const WorldBookGenerateModal: React.FC<WorldBookGenerateModalProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const result = await onGenerateFromCharacters(charInfoText);
      ...
      <div style={{ color: 'var(--text-color, #000)' }}>
                      ...
                      <Checkbox value={char.path}>
                        <div>
                          ...
                          {char.description && (
                            <Paragraph
                              ellipsis={{ rows: 2 }}
                              style={{ marginTop: 8, fontSize: 13, color: 'var(--text-color, #666)' }}
                            >
                              {char.description}
                            </Paragraph>
                          )}
        ...
        {currentStep === 1 && (
          <div>
            ...
            >
              {isGenerating ? 'AI生成中...' : 'AI生成世界书'}
            </Button>

            {generatedEntries.length > 0 && (
              <Card
                title={`已生成 ${generatedEntries.length} 个条目`}
                style={{ marginTop: 16 }}
              >
                      ...
                      <Input.TextArea
                        ...
                        onChange={(e) => {
                          const keys = e.target.value
                            ...
                            .map((k: string) => k.trim())
                            .filter((k: string) => k);
                          handleEntryChange(index, 'key', keys);
                        }}
        ...
        {currentStep === 2 && (
          <div>
            ...
            <Card title={`生成条目（共 ${generatedEntries.length} 个）`} style={{ marginTop: 16 }}>
              {generatedEntries.map((entry, index) => (
                <Card
                  key={index}
                  size="small"
