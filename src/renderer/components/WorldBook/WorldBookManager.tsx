...
// 审核 Modal 集合（单字段 + 一键审核 + 审核结果）
import WorldBookAuditModal from './WorldBookAuditModal';
// Task 8 拆分产物：子组件
...
// Task 17.2: autoGenerated 草稿条目待审阅区
import WorldBookAutoGeneratedReview from './WorldBookAutoGeneratedReview';
...
import WorldBookAuthoringModal from './WorldBookAuthoringModal';
...
const WorldBookManager: React.FC = () => {
  ...
  // ===== 中断 AI 请求 =====
  // 调用后端 ai:cancel IPC 通道切断 HTTP 连接，重置所有 AI 操作状态，
  // 关闭进行中的 AI Modal，确保按钮文案正确恢复。
  const handleCancelAIRequest = useCallback(async () => {
    // 1. 设置本地 ref 中断循环
    isProcessingRef.current = false;
    // 2. 调用后端取消，切断活跃的 HTTP 请求
    try {
      const result = await window.electronAPI.ai.cancel();
      if (result.success) {
        addLog('[WorldBook] 用户中断AI请求，后端已切断HTTP连接', 'warn');
      } else {
        addLog('[WorldBook] 用户中断AI请求，后端无活跃请求可取消', 'warn');
      }
    } catch (e) {
      addLog(`[WorldBook] 用户中断AI请求，调用ai.cancel失败: ${e instanceof Error ? e.message : String(e)}`, 'error');
    }
    // 3. 重置所有 AI 操作状态（恢复按钮文案、关闭 Modal）
    resetAllAIStates();
    // 4. 用户反馈
    message.info('已中断AI请求，后台连接已切断');
    addLog('[WorldBook] 已重置所有AI操作状态', 'warn');
  }, [isProcessingRef, addLog, resetAllAIStates]);

  // ===== Task 17.2: autoGenerated 草稿条目待审阅区 =====
  const [isAutoGeneratedReviewOpen, setIsAutoGeneratedReviewOpen] = useState(false);
  const [autoGeneratedReviewPath, setAutoGeneratedReviewPath] = useState<string | null>(null);

  // ===== 智能体编写世界书 =====
  const { isActive: isAgentModeActive } = useAgentMode();
  const [isAuthoringModalOpen, setIsAuthoringModalOpen] = useState(false);

  /** 打开某本世界书的 autoGenerated 待审阅区 */
  const handleOpenAutoGeneratedReview = useCallback((record: WorldBook) => {
    setAutoGeneratedReviewPath(record.path);
    setIsAutoGeneratedReviewOpen(true);
    addLog(`[WorldBook] 打开 AI 生成条目待审阅区: ${record.name}`);
  }, [addLog]);

  /**
   * 审阅操作完成后刷新本地缓存。
   * 若审阅的世界书正是当前正在查看的（viewingItem），则刷新 worldBookContent。
   */
  const handleAutoGeneratedChanged = useCallback(async () => {
    if (viewingItem && autoGeneratedReviewPath && viewingItem.path === autoGeneratedReviewPath) {
      try {
        ...
        addLog(`[WorldBook] 审阅后刷新世界书失败: ${error instanceof Error ? error.message : '未知错误'}`, 'error');
      }
    }
  }, [viewingItem, autoGeneratedReviewPath, setWorldBookContent, addLog]);

  /**
   * 审核新生成 / 草稿条目：提取 entry.content 设置审核状态并打开审核 Modal。
   * 供 WorldBookAIGenerateFlow 与 WorldBookAutoGeneratedReview 共用。
   */
  const handleAuditGeneratedEntry = useCallback((entry: any) => {
    const content = entry?.content || '';
    if (!content) {
      message.warning('该条目没有内容可审核');
      return;
    }
    addLog(`[WorldBook] 准备审核新生成条目: ${entry?.comment || entry?.name || '未命名'}，内容长度: ${content.length}`);
    setCurrentAuditField('content');
    setCurrentAuditText(content);
    setAuditRequirements('');
    setIsAuditModalOpen(true);
  }, [setCurrentAuditField, setCurrentAuditText, setAuditRequirements, setIsAuditModalOpen, addLog]);
  ...
  // 页面关闭/刷新时终止所有活跃的 AI 请求，避免孤儿请求在后台继续执行
  useEffect(() => {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          {/* Task 17.2: AI 自动生成条目待审阅入口 */}
          <Button
            type="link"
            size="small"
            icon={<RobotOutlined />}
            title="查看 AI 智能体自动生成的草稿条目（待审阅）"
            onClick={() => handleOpenAutoGeneratedReview(record)}
          >
            待审阅
          </Button>
          <Popconfirm
  ...
  ], [handleView, handleVectorizeToWorldBook, handleDelete, handleOpenAutoGeneratedReview]);
    ...
    <div className={`worldbook-manager ${appTheme === 'dark' ? 'dark' : ''}`}>
      <div className="worldbook-header list-header">
        ...
        <Space>
          ...
          {isAgentModeActive && (
            <Button type="primary" icon={<RobotOutlined />} onClick={() => setIsAuthoringModalOpen(true)}>
              智能体编写
            </Button>
          )}
        </Space>
      ...
      <WorldBookEntryTable
        ...
        onAuditAll={aiOps.handleAuditAll}
      ...
      <WorldBookAIGenerateFlow
        ...
        onAuditEntry={handleAuditGeneratedEntry}
      />
      ...
      <WorldBookAuditModal
        ...
        auditingField={auditingField}
        auditRequirements={auditRequirements}
        setAuditRequirements={setAuditRequirements}
        setCurrentAuditField={setCurrentAuditField}
        setCurrentAuditText={setCurrentAuditText}
        performAudit={aiOps.performAudit}
        isAuditAllModalOpen={isAuditAllModalOpen}
        setIsAuditAllModalOpen={setIsAuditAllModalOpen}
        isAuditingAll={isAuditingAll}
        auditAllRequirements={auditAllRequirements}
        setAuditAllRequirements={setAuditAllRequirements}
        performAuditAll={aiOps.performAuditAll}
        isAuditResultModalOpen={isAuditResultModalOpen}
        auditResult={auditResult}
        applyAuditResult={aiOps.applyAuditResult}
        applyAuditResultForBatch={aiOps.applyAuditResultForBatch}
        closeAuditResult={aiOps.closeAuditResult}
        reAudit={aiOps.reAudit}
        isBatchMode={isAuditingAll}
        onCancelAIRequest={handleCancelAIRequest}
      />

      {/* Task 17.2: AI 自动生成条目待审阅区 Modal（世界书自驱） */}
      <WorldBookAutoGeneratedReview
        worldBookPath={autoGeneratedReviewPath}
        open={isAutoGeneratedReviewOpen}
        onClose={() => setIsAutoGeneratedReviewOpen(false)}
        onChanged={handleAutoGeneratedChanged}
        appTheme={appTheme}
        onAuditEntry={handleAuditGeneratedEntry}
      />
