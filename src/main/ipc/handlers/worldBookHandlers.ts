...
import { registerWorldBookAgentHandlers } from './worldbook/worldbookAgentHandlers';
...
export function worldBookHandlers() {
  ...
  // ===== Task 17.2: autoGenerated 草稿条目审阅流 IPC =====
  // 世界书自驱：agent 写入的草稿条目（autoGenerated=true）由用户在 UI 审阅。

  ipcMain.handle('worldBook:listAutoGenerated', async (_event, worldBookPath: string) => {
    return await worldBookService.listAutoGeneratedEntries(worldBookPath);
  });

  ipcMain.handle(
    'worldBook:approveAutoGenerated',
    async (_event, worldBookPath: string, entryUid: string | number) => {
      return await worldBookService.approveAutoGeneratedEntry(worldBookPath, entryUid);
    }
  );

  ipcMain.handle(
    'worldBook:rejectAutoGenerated',
    async (_event, worldBookPath: string, entryUid: string | number) => {
      return await worldBookService.rejectAutoGeneratedEntry(worldBookPath, entryUid);
    }
  );

  ipcMain.handle(
    'worldBook:approveAllAutoGenerated',
    async (_event, worldBookPath: string) => {
      return await worldBookService.approveAllAutoGeneratedEntries(worldBookPath);
    }
  );

  // 注册世界书编写智能体 IPC 通道（run/cancel/status/resume/answer + progress 流）
  registerWorldBookAgentHandlers();
}
