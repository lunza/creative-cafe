...
## 15. 对比结论：creative-cafe 借鉴 SillyTavern 的 AI 智能度优化机制

> 本章节为 2026-07 `optimize-chat-ai-intelligence` spec 实施后追加，记录 creative-cafe 角色对话模块与 SillyTavern 在 AI 智能度核心机制上的对比分析结论。

### 15.1 对比背景

creative-cafe 的角色对话聊天模块（`src/renderer/components/Character/CharacterDialogueChat/`）定位为角色卡创作测试沙箱，与 SillyTavern（生产级 AI 角色扮演前端）相比，在 AI 智能度相关机制上存在 5 项关键差距。本次 `optimize-chat-ai-intelligence` spec（Task 1-8）通过借鉴 SillyTavern v1.18.0 的成熟设计，补齐了这些差距，并补充了 2 项 SillyTavern 未做的应用层机制。
...
| 维度 | 内容 |
|------|------|
| **creative-cafe 原实现** | `ContextTruncator.truncateMessages` 采用"末尾累加 + minMessagesToKeep 回退"算法，预算耗尽时强制回退到最近 N*2 条消息，挤占必填项（system prompt / response 预留）预算 |
| **SillyTavern 借鉴机制** | `ChatCompletion.reserveBudget` / `canAfford` 双向预留机制（源码 `src/endpoints/backends/chat-completions.js` 约 3747 行附近，token 预算按必填项顺序 reserve，剩余预算倒序填充对话历史） |
| **creative-cafe 实现** | `TokenBudget` 类（`ContextTruncator.ts`）：`reserve(key, tokens) / free(key) / canAfford(tokens) / remaining / reserved / total`；必填项顺序 `[systemPrompt, roleAnchor, stopSequenceReserve(512), exampleMessages, responseReserve]`；`minMessagesToKeep` 改为软下限（预算不允许时不强制，spec REMOVED Requirement） |
| **Spec Task** | Task 2 |
...
| 维度 | 内容 |
|------|------|
| **creative-cafe 原实现** | `ChatEngine.ts` 请求体未设置 `stop` 字段，AI 经常自己生成下一条用户消息（"\n用户: ..."）或连续重复角色名/引号 |
| **SillyTavern 借鉴机制** | `names_as_stop_strings` 机制（源码 `public/script.js:2966 getStoppingStrings` 函数定义，`script.js:2974` 检查 `power_user.context.names_as_stop_strings`），将用户名变体（含中英文冒号）作为 stop sequences 注入请求体 |
| **creative-cafe 实现** | `buildStopSequences(userName, customStops?)`（`PromptBuilder.ts:60-84`）返回 `[\n{{user}}:, \n{{user}}：, \n用户:, \n用户：, \nUser:, \nUser：]`；`resolveStopForRequestBody` 纯函数按 `supportsStopArray` 能力解析为数组或字符串；`ParameterPanel.tsx` 新增自定义停止序列 UI（开关 + TextArea） |
| **Spec Task** | Task 3 |
...
| 维度 | 内容 |
|------|------|
| **creative-cafe 原实现** | `PromptBuilder.ts::buildCharacterContext` 只在 system prompt 头部出现一次角色设定，长上下文截断后早期角色设定被"稀释"，AI 性格逐渐漂移（OOC） |
| **SillyTavern 借鉴机制** | `depth_prompt` 机制（源码 `public/script.js:549` `depth_prompt_depth_default = 4` 常量定义，`script.js:4424-4425` 读取角色卡 `data.extensions.depth_prompt` 字段注入对话深处） |
| **creative-cafe 实现** | `buildRoleAnchorMessage(characterCard, userName)`（`PromptBuilder.ts:86-147`）提取 `personality` 前 200 字符构建 `[角色锚定]` system 消息；`ContextTruncator.truncateMessages` 二阶段裁剪——裁剪后对话历史 token > `maxContextTokens * 0.5` 时在 depth=4 位置插入锚定消息；system prompt 末尾追加"角色卡为绝对权威"约束句形成双重保障 |
| **Spec Task** | Task 4 |
...
| 维度 | 内容 |
|------|------|
| **creative-cafe 原实现** | `continueConversation` 无任何续写去重约束，AI 经常原样重写已有内容而非追加新内容 |
| **SillyTavern 借鉴机制** | `continue_nudge_prompt` 机制（源码 `public/scripts/openai.js:110` `default_continue_nudge_prompt` 常量定义，默认值 `[Continue your last message without repeating its original content.]`，在续写请求中作为 system 消息注入） |
| **creative-cafe 实现** | `buildContinueNudgePrompt()`（`PromptBuilder.ts:149-168`）返回 spec 原文；`buildContinuationPrompt` 末尾追加 `【续写去重约束】` 段落（模板路径 + 硬编码回退路径均追加）；重试时通过 `injectContinueNudge=true` 在消息数组末尾追加 nudge system 消息，形成"system prompt 段落 + 消息数组末尾 system 消息"双重提示 |
| **Spec Task** | Task 8 |
...
| 维度 | 内容 |
|------|------|
| **creative-cafe 原实现** | AI 超参数默认值 `frequency_penalty=0` / `presence_penalty=0` / 无 `repetition_penalty` / 无 DRY 采样，采样阶段不防重复 |
| **SillyTavern 借鉴机制** | TextGen WebUI Default 预设 `rep_pen=1.1`（源码 `default/content/presets/textgen/Default.json:12`）+ DRY 采样参数（`dry_multiplier=0.0`/`dry_base=1.75`/`dry_allowed_length=2`，源码 `public/scripts/textgen-settings.js:176-178`；注：SillyTavern 默认 `dry_multiplier=0.0` 即关闭，creative-cafe 调整为 `0.8` 启用） |
| **creative-cafe 实现** | `parameterConfigs.ts` 调整默认值：`top_p=0.95` / `frequency_penalty=0.1` / `presence_penalty=0.1` / `repetition_penalty=1.1`；`ParameterPanel.tsx` 新增 `repetition_penalty` 滑块（按 `supportsRepPen` 能力显隐）+ 高级折叠区 DRY 采样组（按 `supportsDrySampler` 能力显隐）+ 防重复强度三档预设（宽松/标准/严格，见 spec `fix-ai-response-length-degradation`）；`buildSamplingExtras` 纯函数按能力注入请求体 |
| **Spec Task** | Task 6（`optimize-chat-ai-intelligence`）；Task 2+5（`fix-ai-response-length-degradation`，将 `freq_pen/pres_pen` 默认值从 `0.3` 降为 `0.1` 避免与 DRY 叠加过度惩罚导致回复缩短） |

### 15.3 非直接借鉴的补充机制

以下 2 项机制为 creative-cafe 基于 SillyTavern 思路启发但非直接借鉴的原创补充：

#### 补充机制 1：对话历史 RAG 检索

| 维度 | 内容 |
|------|------|
| **creative-cafe 原缺陷** | `ChatVectorizationService` 虽然在每次对话保存后向量化整段对话，但 `requestAIResponse` 从不检索这些向量，向量化的对话历史与 RAG 检索完全脱节，长对话引用早期细节失效 |
| **SillyTavern 启发来源** | `worldinfo.js` World Info 向量检索（World Info 条目支持 `vectorized: true` 走向量检索替代关键字匹配） |
| **creative-cafe 实现** | `retrieveChatHistory(chatId, queryText, topK=3, minScore=0.6)` + `vectorizeIncremental(chatId, messages)`（`ChatVectorizationService.ts`）；`requestAIResponse` 步骤 A2：`contextMessages.length > 40`（> 20 轮）触发检索，结果格式化为"区域 2：本会话相关历史片段"注入 system prompt；`(contextMessages.length + 1) % 10 === 0`（每 5 轮）触发增量向量化 |
| **与 SillyTavern 的差异** | SillyTavern World Info 检索的是**外部世界书条目**（创作者预设的背景知识）；creative-cafe 检索的是**本会话历史消息**（用户与 AI 实际产生的对话），用向量检索替代关键字匹配，召回语义相关而非字面相关的早期对话片段 |
| **Spec Task** | Task 7 |

#### 补充机制 2：n-gram Jaccard 去重检测

| 维度 | 内容 |
|------|------|
| **creative-cafe 原缺陷** | `retryMessage` 与 `continueConversation` 无任何 AI 回复去重/相似度检测，重试常得到几乎相同的回复，续写常原样重写已有内容 |
| **SillyTavern 启发来源** | SillyTavern **不做重试去重**，靠 `repetition_penalty` / DRY 采样等采样参数在生成阶段防重复 |
| **creative-cafe 实现** | `nGramJaccard(textA, textB, n=4)` 字符级 4-gram 集合 Jaccard 相似度 + `overlapRate(newContent, initialContent)` 最长公共前缀比例（`utils/similarityUtils.ts`）；重试时 `nGramJaccard > 0.8` 触发重新生成（最多 2 次），续写时 `overlapRate > 0.6` 触发 `continue_nudge_prompt` 重新生成 |
| **与 SillyTavern 的差异** | SillyTavern 完全依赖采样参数防重复，不在应用层做重试去重；creative-cafe 在采样参数之上**补充应用层去重**——生成完成后用 n-gram Jaccard 量化相似度，超阈值则自动重新生成。该机制为 creative-cafe 原创补充 |
| **Spec Task** | Task 5 |

### 15.4 总结

本次 `optimize-chat-ai-intelligence` spec 共实施 8 项 Task：

- **5 项直接借鉴 SillyTavern**：上下文裁剪（Task 2）、防抢话（Task 3）、角色一致性（Task 4）、续写去重约束（Task 8）、防重复采样（Task 6）
- **2 项非直接借鉴的补充机制**：对话历史 RAG（Task 7）、n-gram Jaccard 去重（Task 5）
- **1 项基础设施**：精确 Token 计数（Task 1，为 Task 2 budget 裁剪提供精确数值基础，中文误差从字节估算的 ±15% 降至 ±3%）

测试覆盖：全量 268 个单元测试通过（Task 1-8 累计新增），所有机制均通过纯函数单测验证决策正确性。⚠️ 受限于无法真实调用 AI API，端到端实际效果验证留待 Task 11 完成。

详细机制说明见 `doc/04b-character-dialogue-chat-module.md` 第 3.3 节"对话智能度优化机制总览"及各机制详细小节。

---

*本文档基于 SillyTavern v1.18.0 源代码（release 分支）完整分析生成，分析深度覆盖 src/ 与 public/scripts/ 下全部 JavaScript 文件及各插件/端点模块。第 15 章节为 2026-07 `optimize-chat-ai-intelligence` spec 实施后追加的对比结论，源码行号已于 2026-07-04 对照 v1.18.0 实际源码校正。*
